...
shinyServer(
  function(input, output){
   ...
   ra.fn <- reactive({
      ...
      tryCatch(eval(parse(text=paste("tryCatch(expr=render.animation(isolate({ca.fn()}),render.par=render.par()",tmp,paste(ralist,sep=",",collapse=","),"),error=function(cond) {cat('Input value is invalid')})")
          )),error=function(e)cat("Input format is invalid"))
     })
   ...
   sa.fn <- reactive({
      ...
      salist <- argFun.sa()
      salist$vedio.name <- "animation.mp4"
